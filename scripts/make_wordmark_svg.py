"""
Render "NANDHU" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
# a very heavy face (Impact) collapses to blobs at this resolution -- the
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">nandagopan@github: ~$ ./wordmark.sh --3d</text>')
